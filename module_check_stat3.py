...
def call_create_service(nfv_vm_id, index):
    ...
    #if r["mode"] == "ON":
        #cur.execute("SELECT * FROM servicechainrule WHERE dpid = %s AND inport = %s", (nfvvm["dpid"], nfvvm["out_port"]))
    #else:
...
if True:
    while True:
        ...
        for nfv in nfv_list:
            ...
            key = nfv["nfv_vm_id"]
            """
            if not prev_pps.has_key(key) or not prev_bps.has_key(key):
                prev_pps[key] = 0.0
                prev_bps[key] = 0.0
            """
            if port.pps <= 0.0:
