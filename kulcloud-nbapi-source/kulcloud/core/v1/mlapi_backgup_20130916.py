...
class mlapi(mlapi_base_v1.MlapiBaseV1):	
    ...
    def get_nfvdb_vm(self, conf):
        ...
        for nfv_id in id_list:
            cur.execute("SELECT * FROM nfv, nfvvm WHERE nfv.nfv_id = nfvvm.nfv_id AND nfv.nfv_id = %s", nfv_id['nfv_id'])
            rows = cur.fetchall()
            for row in rows:
                temp = dict()
                temp['name'] = row['nfv']
        ...
        try:
            ...
            for rule in rule_list:
                ...
                if not row:
                    ...
                    print "[ERROR] : This service level of nfv can not be the default"
                    result['message'] = "FAIL"
                    return result
    ...
    def apply_default_rule(self, conf):
