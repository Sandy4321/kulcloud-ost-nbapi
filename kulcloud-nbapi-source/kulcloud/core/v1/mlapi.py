...
class mlapi(mlapi_base_v1.MlapiBaseV1):	
    ...
    def create_service_wrapper(self, conf, body):
        ...
        if self.get_sdn_mode() == True:
            cur.execute("SELECT nfv_name FROM servicechainlog WHERE chain_id = %s ORDER BY nfv_order", service_chain_id)
            default_list = cur.fetchall()
            nfv_list = dict()
            nfv_list["nfv_list"] = [nfv["nfv_name"] for nfv in default_list]
        else:
    ...
    def delete_service_wrapper(self, conf, service_chain_id):
        try:
            con = mdb.connect("localhost", "root", "of#123", "skdemo2")
            cur = con.cursor(mdb.cursors.DictCursor)
        except:
        ...
        node = self.create_service(conf, phone_num["phone_num"], service_type["service_type"], {"nfv_list":["NAT"]})
