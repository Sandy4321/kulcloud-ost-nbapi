...
class mlapi(mlapi_base_v1.MlapiBaseV1):	
    ...
    def controller_call_for_flowadd(self, cur, chain_id, dpid, sip, dip, in_port, out_port, wildcards, prior, allocIP, src_vlan=0, dst_vlan = 0, is_set = False, is_strip = False, is_voms = False):
        """ Work from Chan"""
        VOMS_MAC = 'f2:d4:e7:1c:56:e1'
        c_dpid = int(dpid, 0)
        db_query = None
        #if not sip:
    ...
    def get_group_statistic(self, conf, nfv_groupname):
        ...
        con.close()
        return {"message" : "SUCCESS", "flows" : [{"nfv_id": row["nfv_id"], "inport_pps": avg_inport_pps, "cpu_usage": avg_cpuusage, "cpu_usage_threshold": threshold["cpu_usage_threshold"], "cpu_usage_duration": threshold["cpu_usage_duration"], "port_pps_threshold": threshold["iport_pps_threshold"], "port_pps_duration": threshold["inport_pps_duration"]}]}
        ...
        for chain in chain_list:
            ...
            while True:
                ...
                try:
                    print "count : %d" % num
                    num = num+1
                    self.controller_call_for_flowadd(cur, chain['chain_id'], forward['dpid'],  ip, None, forward['in_port'], forward['out_port'], s_wildcards, 0, allocIP_src)
                    self.controller_call_for_flowadd(cur, chain['chain_id'], reverse['dpid'],  None, ip, reverse['in_port'], reverse['out_port'], d_wildcards, 0, allocIP_dst)

                    #insert tuple to servicechainrule
                    #cur.execute("INSERT INTO servicechainrule (chain_id, sip, dip, inport, outport, dpid, vlan, wildcard, prior) VALUES (%s, %s, NULL, %s, %s, %s, %s, %s, 0)", (chain['chain_id'], ip, forward['in_port'], forward['out_port'], forward['dpid'], 0, s_wildcards))
                    #cur.execute("INSERT INTO servicechainrule (chain_id, sip, dip, inport, outport, dpid, vlan, wildcard, prior) VALUES (%s, NULL, %s, %s, %s, %s, %s, %s, 0)", (chain['chain_id'], ip, reverse['in_port'], reverse['out_port'], reverse['dpid'], 0, d_wildcards))
                except:
                    #con.rollback()
                    return {"message" : "FAIL"}
            ...
            row = cur.fetchone()
            result['service'].append({"service_type" : row['service_type'], "service_id" : row['service_id']})
