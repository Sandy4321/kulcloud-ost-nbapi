...
class mlapi(mlapi_base_v1.MlapiBaseV1):	
    ...
    def get_stat_port(self, conf, dpid, port_no):
        ...
        flow_list = None
        try:
            flow_list = mul_nbapi.get_switch_statistics_all(int(str(dpid), 0))
        ...
        stat_port = self.stat_port_parse(flow_list, int(str(port_no)))        
        if not stat_port:
    ...
    def create_servicech(self, conf, params):
        ...
        cur.execute("SELECT * FROM user WHERE userip = %s OR phone_num = %s", (user_ip, user_phone_num))
        row = cur.fetchone()
        if not row:
            #insert user db
            ...
            if row["userip"] != user_ip or row["phone_num"] != user_phone_num:
                return {"message" : "FAIL"}
        ...
        for row in rows:
            node = self.create_service(conf, user_phone_num, row["service_type"], default_list)
    ...
    def show_statistic_service_chain(self, conf, service_chain_id):
        ...
        for row in rows:
            ...
            cur.execute("SELECT nfv_name FROM nfvvm WHERE dpid = %s AND (in_port = %s OR out_port = %s)", (c_dpid, out_port, in_port))
            nfvvm = cur.fetchone()
    ...
    def create_service(self, conf, phone_num, service_type, params, updatelog=False):
        ...
        for i in range(len(lnfv)):
            if not lnfv[i]:
                ...
                cur.execute("SELECT * FROM nfv as a, nfvvm as b WHERE a.nfv_id = b.nfv_id AND a.nfv = '%s' AND active = 1 ORDER BY rand() limit 1" % lnfv[i])
                row = cur.fetchone()
                if not row:
        ...
        try:
            ...
            if updatelog is False:
                ...
                #for default in default_list:
                    #cur.execute("INSERT INTO servicechainlog (chain_id, nfv_name, nfv_order) VALUES (%s, %s, %s)", (chain_id, default["nfv"], default["nfv_order"]) )
    ...
    def get_nfvdb_bymdn(self, conf, mdn):
        ...
        cur.execute("SELECT * FROM nfv as a, nfvvm as b WHERE a.nfv_id = b.nfv_id AND b.active = 1 AND a.service_level <= %s ORDER BY b.nfv_id, b.nfv_vm_id", service_level)
        rows = cur.fetchall()
        if not rows:
            return {"message" : "FAIL"}
    ...
    def get_nfvdb_vm(self, conf):
        ...
        for nfv_id in id_list:
            ...
            cur.execute("SELECT * FROM nfv, nfvvm WHERE nfv.nfv_id = nfvvm.nfv_id AND nfv.nfv_id = %s AND nfvvm.active = 1 ORDER BY nfvvm.nfv_id, nfvvm.nfv_vm_id", nfv_id['nfv_id'])
            rows = cur.fetchall()
            if not rows:
                return {"message" : "FAIL"}
    ...
    def get_vm_statistic(self, conf, nfv_name):
        ...
        cur.execute("SELECT b.nfv_id, a.nfv_vm_id, a.in_port_pps, a.cpu_usage FROM nfvvmStatus as a, nfvvm as b WHERE a.nfv_vm_id = b.nfv_vm_id AND b.nfv_name = %s ORDER BY a.time DESC LIMIT 1", nfv_name)
        row = cur.fetchone()

        threshold = self.get_nfv_vm_threshold(conf, row["nfv_id"])

        con.close()
        return {"message" : "SUCCESS", "flow" : [{"nfv_vm_id": row["nfv_vm_id"], "inport_pps": row["in_port_pps"], "cpu_usage": row["cpu_usage"], "cpu_usage_threshold": threshold["cpu_usage_threshold"], "cpu_duration": threshold["cpu_usage_duration"], "inport_pps_threshold": threshold["inport_pps_threshold"], "inport_pps_duration": threshold["inport_pps_duration"]}]}
    ...
    def get_group_statistic(self, conf, nfv_groupname):
        ...
        cur.execute("SELECT a.nfv, b.nfv_id, AVG(c.in_port_pps) as avg_inport_pps, AVG(c.cpu_usage) as avg_cpu_usage FROM nfv as a, nfvvm as b, nfvvmStatus as c WHERE a.nfv_id = b.nfv_id AND b.nfv_vm_id = c.nfv_vm_id AND a.nfv = %s", nfv_groupname )
        row = cur.fetchone()
    ...
    def create_default_rule(self, conf, body):
        ...
        if nfv_list[-1]["nfv"] != "VOMS" and nfv_list[-1]["nfv"] != "NAT":
            nfv_list.append({"nfv":"NAT"})
    ...
    def create_service_by_chain_log(self, conf, params, admin=True):
        ...
        #result["mdn"] = user["phone_num"]
        #result["service_level"] = user["service_level"]
        ...
        for sa in sa_list:
            ...
            if not log_list:
                ...
                #cur.execute("SELECT b.nfv_name, a.nfv_order FROM defaultRule as a, nfvvm as b, nfv as c WHERE a.nfv = c.nfv AND c.nfv_id = b.nfv_id ORDER BY a.nfv_order")
                cur.execute("SELECT * FROM defaultRule ORDER BY nfv_order")
                rows = cur.fetchall()
                #prev_order = 0
                for row in rows:
                    #if prev_order == row["nfv_order"]:
                        #continue

                    #prev_order = row["nfv_order"]
                    params["nfv_list"].append(row["nfv"])
            else:
                for log in log_list:
