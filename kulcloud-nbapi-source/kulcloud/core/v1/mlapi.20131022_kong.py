...
class mlapi(mlapi_base_v1.MlapiBaseV1):	
    ...
    def show_statistic_service_chain(self, conf, service_chain_id):
        ...
        for row in rows:
            ...
            """ 2013-10-22 modified """
            nfvvm = [vm for vm in self.vm_map if vm["dpid"] == c_dpid and (vm["in_port"] == in_port or vm["in_port"] == out_port or vm["out_port"] == in_port or vm["out_port"] == out_port)]
    ...
    def create_nfvdb(self, conf, params):
        ...
        if not nfvvm:
            ...
            if not row:
                ...
                if row["dpid"] != nfv_dpid or row["in_port"] != nfv_inport or row["out_port"] != nfv_outport:
                    return {"message" : "Dpid and port is already using"}
        ...
        if make_new_node:
            ...
            cur.execute("INSERT INTO nfvvm (nfv_id, nfv_vm_id, nfv_name, dpid, in_port, out_port, active, active_threshold, vlan) VALUES (%s, %s, %s, %s, %s, %s, %s, 1, 0, %s)", (new_id["nfv_id"]+1, new_vm_id["nfv_vm_id"]+1, nfv_vm_name, nfv_dpid, nfv_inport, nfv_outport, new_vlan+1))

        if update_node:
            cur.execute("UPDATE nfvvm SET dpid = %s, in_port = %s, out_port = %s WHERE nfv_name = %s", (nfv_dpid, nfv_inport, nfv_outport, nfv_vm_name))
