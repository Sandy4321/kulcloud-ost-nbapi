...
class Controller(object):
    def __init__(self, conf):
        LOG.debug("Creating Flow Manager controller with config:"
                                                "loadkulclouds.py %s", conf)
        self.conf = conf
        self.name = 'FlowManager'

    @utils.verify_version
    def index(self, req, version, dpid):
        LOG.debug("Got index request. Request: %s", req)
        result = core_api.flow_get_index(self.conf, version, dpid)
        return result
    ...
    def create(self, req, version, dpid, body):
        ...
        result = core_api.create_flow(self.conf, version, dpid, body.copy())
        return result
    ...
    def delete(self, req, version, dpid, flow_id):
        ...
        try:
            result = core_api.delete_flow(self.conf, version, dpid, flow_id)
    ...
    def show(self, req, version, dpid, flow_id):
        ...
        result = core_api.flow_get_data(self.conf, version, dpid, flow_id)
        return result

    @utils.verify_version
    def details(self, req, version, dpid, flow_id):
        LOG.debug("Got loadkulcloudr info request. Request: %s", req)
        result = core_api.flow_show_details(self.conf, version, dpid, flow_id)
        return result
    ...
    def update(self, req, version, dpid, flow_id, body):
        ...
        params = body.copy()
        result = core_api.update_flow(self.conf, version, dpid, flow_id, params)
        return result


def create_resource(conf):
    """FlowManager resource factory method"""
