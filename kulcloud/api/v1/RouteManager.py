...
class Controller(object):
    def __init__(self, conf):
        LOG.debug("Creating route Manager controller with config:"
    ...
    def index(self, req, version):
        ...
        result = core_api.route_get_index(self.conf, version)
        return result
    ...
    def create(self, req, version, body):
        ...
        result = core_api.create_route(self.conf, version, params)
        return result

    @utils.http_success_code(204)
    @utils.verify_version
    def delete(self, req, version, flow_id):
        LOG.debug("Got delete request. Request: %s", req)
        result = core_api.delete_route(self.conf, version, flow_id)
        return result

    @utils.verify_version
    def show(self, req, version, flow_id):
        LOG.debug("Got loadkulcloudr info request. Request: %s", req)
        result = core_api.route_get_data(self.conf, version, flow_id)
        return result

    @utils.verify_version
    def details(self, req, version, flow_id):
        LOG.debug("Got loadkulcloudr info request. Request: %s", req)
        result = core_api.route_show_details(self.conf, version, flow_id)
        return result

    @utils.http_success_code(202)
    @utils.verify_version_argument
    def update(self, req, version, lb_id, body):
        LOG.debug("Got update request. Request: %s", req)
        result = core_api.update_route(self.conf, version, lb_id, body)
        return result


def create_resource(conf):
    """routeManager resource factory method"""
