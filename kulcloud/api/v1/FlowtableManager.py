...
class Controller(object):
    def __init__(self, conf):
        LOG.debug("Creating flowtable Manager controller with config:"
    ...
    def index(self, req, version):
        ...
        result = core_api.flowtable_get_index(self.conf, version)
        return result
    ...
    def create(self, req, version, body):
        ...
        result = core_api.create_flowtable(self.conf, version, params)
        return result
    ...
    def delete(self, req, version, dpid):
        ...
        result = core_api.delete_flowtable(self.conf, version, dpid)
        return result
    
    @utils.verify_version
    def show(self, req, version, dpid):
        LOG.debug("Got loadkulcloudr info request. Request: %s", req)
        result = core_api.flowtable_get_data(self.conf, version, dpid)
        return result
    
    @utils.verify_version
    def details(self, req, version, dpid):
        LOG.debug("Got loadkulcloudr info request. Request: %s", req)
        result = core_api.flowtable_show_details(self.conf, version, dpid)
        return result
    ...
    def update(self, req, version, dpid, body):
        ...
        result = core_api.update_flowtable(self.conf, version, dpid, body)
        return result


def create_resource(conf):
    """flowtableManager resource factory method"""
