...
class Controller(object):
    ...
    def index(self, req, version, dpid):
        ...
        result = core_api.stats_flow_get_index(self.conf, version, dpid)
        return result
    ...
    def create(self, req, version, dpid, body):
        ...
        result = core_api.create_stats_flow(self.conf, version, dpid, params)
        return result

    @utils.http_success_code(204)
    @utils.verify_version
    def delete(self, req, version, dpid, flow_id):
        LOG.debug("Got delete request. Request: %s", req)
        result = core_api.delete_stats_flow(self.conf, version, dpid, flow_id)
        return result

    @utils.verify_version
    def show(self, req, version, dpid, flow_id):
        LOG.debug("Got Stats Manager info request. Request: %s", req)
        result = core_api.stats_flow_get_data(self.conf, version, dpid, flow_id)
        return result

    @utils.verify_version
    def details(self, req, version, dpid, flow_id):
        LOG.debug("Got Stats Manager info request. Request: %s", req)
        result = core_api.stats_flow_show_details(self.conf, version, dpid, flow_id)
        return result

    @utils.http_success_code(202)
    @utils.verify_version_argument
    def update(self, req, version, dpid, flow_id, body):
        LOG.debug("Got update request. Request: %s", req)
        result = core_api.update_stats(self.conf, version, dpid, flow_id, body)
        return result
    
    @utils.http_success_code(202)
    @utils.verify_version_argument
    def show_stats_port(self, req, version, dpid, port_no):
        LOG.debug("Got Port stats request. Request: %s", req)
        result = core_api.stats_port_get_index(self.conf, version, dpid, port_no)
        return result
