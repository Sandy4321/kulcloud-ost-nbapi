...
class mlapi(mlapi_base_v1.MlapiBaseV1):	
    ...
    def create_servicech(self, conf, params):
        ...
        #cur.execute("SELECT service_id FROM ippool WHERE user_id IN (SELECT userid FROM user WHERE userip = %s)", self.default_user_ip)
        #default_ippool_list = cur.fetchall()

        default_list = {"nfv_list" : []}
        for default in defaults:
            default_list["nfv_list"].append(default["nfv"])

        #for ippool in default_ippool_list:
        for row in rows:
            #cur.execute("SELECT * FROM servicechainlog WHERE chain_id = %s", service["service_id"])
            #default_list = cur.fetchall()
            #nfv_list = {"nfv_list" : []}
            #for default in default_list:
                #nfv_list["nfv_list"].append(default["nfv_name"])
            
            #cur.execute("SELECT * FROM sa WHERE service_id = %s", service["service_id"]
            #service = cur.fetchone()
            node = self.create_service(conf, user_phone_num, service["service_type"], default_list)
    ...
    def create_service(self, conf, phone_num, service_type, params, updatelog=False):
        ...
        cur.execute("SELECT * FROM user WHERE phone_num = %s AND active = 1", user_phone_num)
        row = cur.fetchone()
        ...
        for i in range(len(lnfv)):
            if not lnfv[i]:
                ...
                # 1. Using nfv_name, nfv_id find nfv_list with nfv_vm_id and thresh hold value
                ...
                # 1.1 order the nfv_list 
                # 2. for nfv_vm_id in nfv_list:
                # 3.  if out_port_pps > thresh hold : pass
                # 4. Find the nfv from nfvvm DB
    ...
    def get_vm_statistic(self, conf, nfv_name):
        ...
        cur.execute("SELECT b.nfv_id, a.nfv_vm_id, a.out_port_pps, a.cpu_usage FROM nfvvmStatus as a, nfvvm as b WHERE a.nfv_vm_id = b.nfv_vm_id AND b.nfv_name = %s ORDER BY a.time DESC LIMIT 1", nfv_name)
        row = cur.fetchone()
        ...
        con.close()
        return {"message" : "SUCCESS", "flow" : [{"nfv_vm_id": row["nfv_vm_id"], "inport_pps": row["out_port_pps"], "cpu_usage": row["cpu_usage"], "cpu_usage_threshold": threshold["cpu_usage_threshold"], "cpu_duration": threshold["cpu_usage_duration"], "inport_pps_threshold": threshold["inport_pps_threshold"], "inport_pps_duration": threshold["inport_pps_duration"]}]}
    ...
    def get_group_statistic(self, conf, nfv_groupname):
        ...
        cur.execute("SELECT a.nfv, b.nfv_id, SUM(c.out_port_pps) as avg_inport_pps, AVG(c.cpu_usage) as avg_cpu_usage FROM nfv as a, nfvvm as b, nfvvmStatus as c WHERE a.nfv_id = b.nfv_id AND b.nfv_vm_id = c.nfv_vm_id AND a.nfv = %s AND b.active = 1", nfv_groupname )
        row = cur.fetchone()

        avg_inport_pps = float(row["avg_inport_pps"])
