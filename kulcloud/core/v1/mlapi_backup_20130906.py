...
class mlapi(mlapi_base_v1.MlapiBaseV1):	
    ...
    def controller_call_for_flowadd(self, dpid, sip, dip, in_port, out_port, wildcards, prior, allocIP, src_vlan=0, dst_vlan = 0, is_set = False, is_strip = False):
        """ Work from Chan"""
        dpid = int(dpid, 0)
        #if not sip:
        ...
        #wildcards = mul_nbapi.OFPFW_ALL & ~(mul_nbapi.OFPFW_NW_SRC_MASK) & ~(mul_nbapi.OFPFW_DL_TYPE) & ~(mul_nbapi.OFPFW_NW_DST_MASK)
        resp = mul_nbapi.add_static_flow(dpid, flow, wildcards, prior, action_list)
        ...
        for row in rows:
            ...
            while True:
                ...
                if s_dpid == dest_dpid:
                    ...
                    self.controller_call_for_flowadd(s_dpid, s_uniqueip, None, s_s_inport, s_s_outport, s_wildcards, 0, allocIP_src)
                    self.controller_call_for_flowadd(s_dpid, None, d_uniqueip, s_d_inport, s_d_outport, d_wildcards, 0, allocIP_dst)
                    #rule DB Add
            ...
            if not lnfv[i]:
                ...
                while True:
                    ...
                    if s_dpid == endnfv_dpid:
                        ...
                        if is_vlan_tagging:
                            print "sip:%s, dip:%s, dpid:%s, vlan:%s, in_port:%s, out_port:%s" % (s_serviceIP, 0, s_dpid, src_nfv_vlan, s_s_inport, s_s_outport)
                            ...
                            self.controller_call_for_flowadd(s_dpid, s_serviceIP, None, s_s_inport, s_s_outport, s_wildcards, 1, allocIP_src, src_nfv_vlan, dst_nfv_vlan)
                            self.controller_call_for_flowadd(s_dpid, None, d_serviceIP, s_d_inport, s_d_outport, d_wildcards, 1, allocIP_dst, dst_nfv_vlan, src_nfv_vlan, True)
                            #rule DB Add
                            ...
                            self.controller_call_for_flowadd(s_dpid, s_serviceIP, None, s_s_inport, s_s_outport, s_wildcards, 1, allocIP_src)
                            self.controller_call_for_flowadd(s_dpid, None, d_serviceIP, s_d_inport, s_d_outport, d_wildcards, 1, allocIP_dst, 0, 0, False, is_strip_f)
                            #rule DB Add
                    ...
                    if is_vlan_tagging:
                        print "sip:%s, dip:%s, dpid:%s, vlan:%s, in_port:%s, out_port:%s, is_set:%s" % (s_serviceIP, 0, s_dpid, src_nfv_vlan, s_s_inport, s_s_outport, is_set)
                        ...
                        self.controller_call_for_flowadd(s_dpid, s_serviceIP, None, s_s_inport, s_s_outport, s_wildcards, 1, allocIP_src, src_nfv_vlan, dst_nfv_vlan, is_set)
                        if ((s_dpid == start_dpid) and (i==0)) :
                            self.controller_call_for_flowadd(s_dpid, None, d_serviceIP, s_d_inport, s_d_outport, d_wildcards, 1, allocIP_dst, src_nfv_vlan, dst_nfv_vlan)
                        else :
                            self.controller_call_for_flowadd(s_dpid, None, d_serviceIP, s_d_inport, s_d_outport, d_wildcards, 1, allocIP_dst, dst_nfv_vlan, src_nfv_vlan)
                        ...
                        self.controller_call_for_flowadd(s_dpid, s_serviceIP, None, s_s_inport, s_s_outport, s_wildcards, 1, allocIP_src, 0)
                        self.controller_call_for_flowadd(s_dpid, None, d_serviceIP, s_d_inport, s_d_outport, d_wildcards, 1, allocIP_dst, 0)
                        #rule DB Add
                        cur.execute("INSERT INTO servicechainrule (chain_id, sip, dip, inport, outport, dpid, vlan, wildcard, prior) VALUES (%s, %s, NULL, %s, %s, %s, 0, %s, 1), (%s, NULL, %s, %s, %s, %s, 0, %s, 1)", (chain_id, s_serviceIP, s_s_inport, s_s_outport, s_dpid, s_wildcards, chain_id, d_serviceIP, s_d_inport ,s_d_outport, s_dpid, d_wildcards))
                        if is_strip_l:
                            is_strip_l = False
    ...
    def controller_call_for_flowdel(self, dpid, sip, dip, in_port, out_port, prior, vlan):
        """ Work from Chan"""
        dpid = int(dpid, 0)
        if not sip:
            wildcards = mul_nbapi.OFPFW_ALL & ~(mul_nbapi.OFPFW_NW_DST_MASK) & ~(mul_nbapi.OFPFW_DL_TYPE) & ~(mul_nbapi.OFPFW_IN_PORT)
        ...
        #        action_list.append(mul_nbapi.nbapi_make_action_set_nw_saddr(str(allocIP)))
        #action_list.append(mul_nbapi.nbapi_make_action_output(int(out_port)))       
        resp = mul_nbapi.delete_static_flow(dpid, flow, wildcards, int(out_port), int(prior))
    ...
    def get_all_nfvdb(self,conf):
        ...
        cur.execute("SELECT * FROM nfv as a, nfvvm as b WHERE a.nfv_id = b.nfv_id")
        rows = cur.fetchall()
        ...
        for row in rows:
            ...
            result["nfv_list"].append({"name":row["nfv"],"type":row["type"], "service_level":row["service_level"],"nfv_name":row["nfv_name"],"DPID":row["dpid"],"IN_PORT":row["in_port"],"OUT_PORT":row["out_port"]})
...
def calculate_path_by_default_rule(conf):
    try:
    ...
    for service in service_list:
        ...
        start_path = dict()
        start_path['dpid'] = service['dpid']
        start_path['in_port'] = service['in_port']
        start_path['out_port'] = service['out_port']
        cur.execute("SELECT vlan FROM nfvvm WHERE dpid = %s LIMIT 1", start_path['dpid'])
        row = cur.fetchone()

        #if start switch have not vlan value, then the start_path's vlan value is temporarily zero
        if not row:
            vlan_num = 0
        else:
            vlan_num = row['vlan']
        start_path['vlan'] = vlan_num
        path_list.append(start_path)

        #before calculate path then the dest_nfv's vlan value is temporarily zero
        dest_nfv = dict()
        dest_nfv['dpid'] = start_path['dpid']
        dest_nfv['vlan'] = 0

        path = start_path
        dRule_iter = defaultRule_list.__iter__()
        prev_vlan = start_path['vlan']
        while True:
            if dest_nfv['dpid'] == path['dpid']:
                try:
                    dest_nfv = dRule_iter.next()
                except StopIteration:
                    break

            cur.execute("SELECT * FROM topo WHERE dpid = %s AND dst_dpid = %s", (path['dpid'], dest_nfv['dpid']))
            row = cur.fetchone()

            try:
                next_path = dict()
                next_path['dpid'] = row['nexthop_dpid']
                next_path['in_port'] = row['in_port']
                next_path['out_port'] = row['out_port']
                next_path['vlan'] = dest_nfv['vlan']

                #Calculate reverse_path node with next path
                #If switch is changed in_port and out_port, this code must modified.
                reverse_path = dict()
                reverse_path['dpid'] = next_path['dpid']
                reverse_path['in_port'] = next_path['out_port']
                reverse_path['out_port'] = next_path['in_port']
            except Exception as err:
                if path['dpid'] == dest_nfv['dpid']:
                    continue
                else:
            ...
            reverse_path['vlan'] = prev_valn
            prev_vlan = next_path['vlan']
            reverse_path_list.append(reverse_path)

        #Complete to calculate reverse path
        reverse_path_list.reverse()
        reverse_path_list.append(start_path)

        all_service_path_list['forward'].append(path_list)
