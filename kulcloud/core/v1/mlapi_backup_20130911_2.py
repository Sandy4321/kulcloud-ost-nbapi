...
class mlapi(mlapi_base_v1.MlapiBaseV1):	
    ...
    def controller_call_for_flowadd(self, cur, chain_id, dpid, sip, dip, in_port, out_port, wildcards, prior, allocIP, src_vlan=0, dst_vlan = 0, is_set = False, is_strip = False):
        """ Work from Chan"""
        c_dpid = int(dpid, 0)
        db_query = None
        #if not sip:
    ...
    def apply_default_rule(conf):
        ...
        cur.execute("SELECT * FROM user, ippool WHERE user.userid = ippool.user_id")
        user_list = cur.fetchall()

        for user in user_list:
            service_num = user['service_id'] - 1
            ...
            while True:
                ...
                #self.controller_call_for_flowadd(forward['dpid'], ip, None, forward['in_port'], forward['out_port'], s_wildcards, 0, allocIP_src)
                #self.controller_call_for_flowadd(reverse['dpid'], None, ip, reverse['in_port'], reverse['out_port'], d_wildcards, 0, allocIP_dst)
