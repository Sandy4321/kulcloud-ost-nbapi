...
class mlapi(mlapi_base_v1.MlapiBaseV1):	
    ...
    def show_switch_list(self, conf):
        ...
        #cur.execute("SELECT switch.dpid, switch.ip, sa.service_id, sa.service_type, sa.in_port, sa.out_port, nfv.nfv FROM switch, sa, nfvvm, nfv WHERE switch.dpid = nfvvm.dpid AND nfvvm.dpid = sa.dpid AND nfvvm.nfv_id = nfv.nfv_id ORDER BY sa.service_id ASC")
        rows = cur.fetchall()
        result = dict()
        result["switch_list"] = []

        for row in rows:
            #result["switch_list"].append({"dpid":row["dpid"], 
            #                              "nfv":row["nfv"],
            #                              "ip":row["ip"],
            #                              "service_type":row["service_type"],
            #                              "port":row["in_port"]
    ...
    def calculate_path_by_default_rule(conf):
        ...
        for service in service_list:
            ...
            while True:
                ...
                if dest_nfv['dpid'] == path['dpid']:
                    ...
                    try:
                        ...
                        forward_path['vlan'] = dset_nfv['vlan']
                        reverse_path['vlan'] = curr_vlan
                ...
                cur.execute("SELECT * FROM topo WEHRE dpid = %s AND dst_dpid = %s", (next_node['nexthop_dpid'], curr_node['dpid']))
            ...
            all_service_paty_list['reverse'].append(reverse_path_list)

        con.close()
        return all_service_path_list

    def apply_default_rule(conf):
        try:
            con = mdb.connect("localhost", "root", "z2028757", "skdemo")
            cur = con.cursor(mdb.cursors.DictCursor)
        except:
        ...
        #all_path = calculate_path_to_default_rule(conf)
